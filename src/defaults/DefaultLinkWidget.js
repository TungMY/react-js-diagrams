...
export class DefaultLinkWidget extends React.Component {
    ...
    generateLink(extraProps) {
        ...
        const bottom = <path className={selected || link.isSelected() ? 'selected' : ''} strokeWidth={width} stroke={color} {...extraProps} />
    ...
    drawLine() {
        ...
        paths.push(
            this.generateLink({
                ...
                d: ` M${pointLeft.x} ${pointLeft.y} C${pointLeft.x + margin} ${pointLeft.y} ${pointRight.x - margin} ${pointRight.y} ${pointRight.x} ${
                    pointRight.y
                }` // eslint-disable-line
            })
        )
    ...
    drawAdvancedLine() {
        ...
        if (smooth) {
            ...
            ds.push(
                ` M${points[i].x} ${points[i].y} C ${points[i].x} ${points[i].y} ${points[i + 1].x - 50} ${points[i + 1].y} ${points[i + 1].x} ${
                    points[i + 1].y
                }` // eslint-disable-line
            )
        } else {
