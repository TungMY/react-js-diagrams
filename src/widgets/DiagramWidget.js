...
export class DiagramWidget extends React.Component {
    ...
    onMouseMove(event) {
        ...
        if (action instanceof SelectingAction && multiselectDrag) {
            ...
            diagramModel.setOffset(
                action.initialOffsetX + (event.pageX - left - this.state.action.mouseX) / (diagramModel.getZoomLevel() / 100),
                action.initialOffsetY + (event.pageY - top - this.state.action.mouseY) / (diagramModel.getZoomLevel() / 100)
            )
    ...
    ondblclick(event) {
        ...
        if (model) {
            ...
            ;(isLink || isNode) && diagramModel.dblClicked(model)
            isPoint && diagramModel.dblClicked(model)
        }
    }
    ...
    render() {
        ...
        return (
            <div
                ...
                onDoubleClick={this.ondblclick.bind(this)}
            >
